...
class SendUsersReport extends Command
{
    protected $signature = 'usersReport:send';
    protected $description = 'Генерация и отправка Excel-отчета по представителям';

public function handle()
{
    $filePath = 'reports/users_report.xlsx';

    // 1) на всякий случай создаём папку
    Storage::disk('local')->makeDirectory('reports');

    // 2) генерим Excel
    $ok = Excel::store(new UsersReportExport, $filePath, 'local');
    if (!$ok) {
